...
class tau_event:
    ...
    def event_energy_cut(self):
        tau_energy = np.asarray([self.e_set for x in range(self.N)])
...
#                 val = 0 
#                 exit_p_arr.append(val)
        exit_p_arr = np.asarray([1 for x in range(len(exit_angle_arr))])
        return exit_p_arr
...
#         return p_decay_arr
    ...
    def event_retention(self):
        ...
        print self.N, len(ret_exit_decay), len(ret_zenith)
